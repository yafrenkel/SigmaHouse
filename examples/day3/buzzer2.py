# -*- coding: utf-8 -*-
"""Stand-alone module playing parametrized melody with buzzer."""
...
def play_tone_for_duration(buzzer, tone, duration, active_duty=512):
...
def play_melody(buzzer, tones, rhythm, tempo):
    """Play melody with buzzer.

    param tones: list of tones
    param rhythm: list of rhythm note dividers (if 1/16 dividers is 16, etc)
    param tempo: length of whole note (ms)
    """
    if len(tones) != len(rhythm):
        raise ValueError("Rhythm and tones must have the same length.")

    for tone, duration in zip(tones, rhythm):
        play_tone_for_duration(buzzer, tone, int(tempo / duration))
